...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_SSOT_DEMO_xxx
namespace = Collection("nautobot_ssot_demo")
namespace.configure(
    {
        "nautobot_ssot_demo": {
            "nautobot_ver": "2.3.1",
            "project_name": "nautobot-ssot-demo",
            "python_ver": "3.11",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_ssot_demo.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_ssot_demo.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_ssot_demo.nautobot_ver,
        "PYTHON_VER": context.nautobot_ssot_demo.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_ssot_demo.project_name}",
        f'--project-directory "{context.nautobot_ssot_demo.compose_dir}"',
    ]

    for compose_file in context.nautobot_ssot_demo.compose_files:
        compose_file_path = os.path.join(context.nautobot_ssot_demo.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_ssot_demo.local):
        ...
        if service in results.stdout:
            ...
            # In Windows, command is not correctly interpreted when it has single quotes.
            entrypoint = f"--entrypoint=\\\"{command}\\\"" if sys.platform == "win32" else f"--entrypoint='{command}'"
            compose_command = f"run{command_env_args} --rm {entrypoint} {service}"
        pty = kwargs.pop("pty", sys.platform != "win32")        
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_ssot_demo.python_ver}...")
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        os.path.join(context.nautobot_ssot_demo.compose_dir, "creds.env")
    ):
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_ssot_demo.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_ssot_demo.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_ssot_demo.python_ver
    dockerfile_path = os.path.join(context.nautobot_ssot_demo.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_ssot_demo.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_ssot_demo.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_ssot_demo.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_ssot_demo"
...
@task(
    help={
        "version": "Version of Nautobot Ssot Demo to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_ssot_demo"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_ssot_demo") / Path("migrations")
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_ssot_demo.local):
